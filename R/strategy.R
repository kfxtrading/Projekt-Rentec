...
default_strategy_config <- function() {
  list(
    ...
    # When TRUE, after the regular full-history backtest the pipeline runs
    # an expanding-window walk-forward: each fold trains the ML/MM models
    # on data up to the fold cutoff and evaluates on the next slice. The
    # initial Phase 1 parameter search is NOT re-run per fold (too costly).
    walk_forward = TRUE,
    wf_n_folds = 5L,
    # Embargo (trading bars) between train cutoff and test start to avoid
    # leakage from overlapping holding-period returns. Defaults to
    # holding_period when NULL.
    wf_embargo_bars = NULL,
    # Skip walk-forward entirely if fewer ml_data rows than this threshold.
...
run_final_backtest <- function(all_data, best_params, lot_size_model, config, output_dir,
                               mm_models = NULL, trade_window = NULL,
                               starting_capital = NULL,
                               output_basename = "final_trades") {
  cat("Starting final backtest...\n")

  trades <- list()
  current_capital <- if (!is.null(starting_capital)) starting_capital else config$initial_capital

  win_start <- if (!is.null(trade_window)) as.Date(trade_window[1L]) else NULL
  win_end   <- if (!is.null(trade_window)) as.Date(trade_window[2L]) else NULL
  ...
  if (nrow(all_data) <= config$holding_period + 1L) {
    ...
    saveRDS(trade_data, file.path(output_dir, paste0(output_basename, ".rds")))
    utils::write.csv(trade_data, file.path(output_dir, paste0(output_basename, ".csv")), row.names = FALSE)
    return(trade_data)
  }
  ...
  for (i in 2:(nrow(all_data) - config$holding_period)) {
    ...
    if (!is.null(win_start)) {
      bar_date <- as.Date(zoo::index(all_data)[i])
      if (bar_date < win_start || bar_date > win_end) next
    }
...
# Compute per-fold OOS metrics from a trade frame produced by run_final_backtest.
...
# Expanding-window walk-forward CV over the ML/MM stage. Phase 1 best params
# stay fixed (single-pass on full history). For each fold:
#   * train ML and money-management models on ml_data[1..cut_idx[k]]
#   * backtest run_final_backtest restricted to (test_start_date, test_end_date]
#   * embargo of `wf_embargo_bars` trading bars between train and test
#   * the running OOS equity chains across folds, starting from
#     config$initial_capital
run_walk_forward <- function(all_data, ml_data, best_params, config, output_dir) {
  ...
  fold_config <- config
  fold_config$resume_checkpoint <- FALSE  # no cache reuse across folds
  ...
  for (k in seq_len(n_segments)) {
    ...
    if (k == n_segments) {
      test_end_date <- ml_dates[length(ml_dates)]
    } else {
      test_end_date <- ml_dates[cut_idx[k + 1L]]
    }

    # Embargo: skip embargo_bars trading days after train_end_date.
